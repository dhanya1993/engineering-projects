...
export function ContactDetailModal({ contact, onClose, onDeleted }: ContactDetailModalProps) {
  ...
  function load() {
    setLoading(true);
    api
      .get<Activity[]>("/activities", { params: { contactId: contact._id } })
  ...
  async function handleAddActivity(e: FormEvent) {
    ...
    await api.post("/activities", { contactId: contact._id, type, content });
    setContent("");
    load();
  }
      ...
      >
        <div className="border-b border-slate-100 px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="font-display text-lg font-semibold text-slate-900">{contact.name}</h2>
